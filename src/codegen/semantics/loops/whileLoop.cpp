...
namespace Cryo
{
    ...
    {
        DevDebugger::logMessage("INFO", __LINE__, "WhileStatements", "Handling While Loop");

        // Get the condition
        ASTNode *condition = node->data.whileStatement->condition;
        if (!condition)
        {
            DevDebugger::logMessage("ERROR", __LINE__, "WhileStatements", "Condition not found");
            CONDITION_FAILED;
        }
        ...
        if (!block)
        {
            DevDebugger::logMessage("ERROR", __LINE__, "WhileStatements", "Block not found");
            CONDITION_FAILED;
        }
        ...
        if (!conditionValue)
        {
            DevDebugger::logMessage("ERROR", __LINE__, "WhileStatements", "Condition value not found");
            CONDITION_FAILED;
        }
        ...
        DevDebugger::logMessage("INFO", __LINE__, "WhileStatements", "While Loop Handled");
