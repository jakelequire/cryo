...
ASTNode *parseProgram(Lexer *lexer, CryoSymbolTable *table, Arena *arena, CompilerState *state)
{
    ...
    while (lexer->currentToken.type != TOKEN_EOF)
    {
        ...
        else
        {
            ...
            parsingError("Failed to parse statement.", "parseProgram", table, arena, state, lexer, source);
            return NULL;
        }
...
void consume(Lexer *lexer, CryoTokenType type, const char *message, const char *functionName, CryoSymbolTable *table, Arena *arena, CompilerState *state)
{
    ...
    {
        parsingError((char *)message, (char *)functionName, table, arena, state, lexer, source);
    }
...
char *getNamespaceName(Lexer *lexer, Arena *arena, CompilerState *state)
{
    ...
    {
        parsingError("Expected a namespace name", "getNamespaceName", NULL, arena, state, lexer, source);
    }
...
CryoDataType getCryoDataType(const char *typeStr, Arena *arena, CompilerState *state, Lexer *lexer)
{
    ...
    {
        parsingError("Unknown data type", "getCryoDataType", NULL, arena, state, lexer, source);
    }
...
CryoDataType parseType(Lexer *lexer, ParsingContext *context, CryoSymbolTable *table, Arena *arena, CompilerState *state)
{
    ...
    default:
        parsingError("Expected a type identifier", "getNextToken", table, arena, state, lexer, source);
        break;
    }
...
ASTNode *parseStatement(Lexer *lexer, CryoSymbolTable *table, ParsingContext *context, Arena *arena, CompilerState *state)
{
    ...
    case TOKEN_KW_THIS:
    {
        ...
        return parseThisContext(lexer, table, context, arena, state);
    }
    ...
    default:
        parsingError("Expected a statement", "parseStatement", table, arena, state, lexer, source);
        return NULL;
    }
}
// </parseStatement>
...
ASTNode *parseScopeCall(Lexer *lexer, CryoSymbolTable *table, ParsingContext *context, Arena *arena, CompilerState *state)
{
    ...
    if (!symbol)
    {
        parsingError("Symbol not found", "parseScopeCall", table, arena, state, lexer, source);
    }
...
ASTNode *parseScopedFunctionCall(Lexer *lexer, CryoSymbolTable *table, ParsingContext *context, Arena *arena, CompilerState *state, const char *functionName, const char *scopeName)
{
    ...
    while (lexer->currentToken.type != TOKEN_RPAREN)
    {
        ...
        {
            parsingError("Failed to parse argument", "parseScopedFunctionCall", table, arena, state, lexer, source);
        }
...
ASTNode *parseNamespace(Lexer *lexer, CryoSymbolTable *table, ParsingContext *context, Arena *arena, CompilerState *state)
{
    ...
    {
        parsingError("Expected a namespace name", "parseNamespace", table, arena, state, lexer, source);
    }
...
ASTNode *parsePrimaryExpression(Lexer *lexer, CryoSymbolTable *table, ParsingContext *context, Arena *arena, CompilerState *state)
{
    ...
    case TOKEN_BOOLEAN_LITERAL:
    {
        ...
        {
            parsingError("Invalid boolean value", "parsePrimaryExpression", table, arena, state, lexer, source);
        }
    ...
    default:
    {
        ...
        parsingError("Expected an expression", "parsePrimaryExpression", table, arena, state, lexer, source);
        return NULL;
    }
    }
    ...
    {
        node = parseDotNotation(lexer, table, context, arena, state, node);
    }
...
ASTNode *parseBinaryExpression(Lexer *lexer, CryoSymbolTable *table, ParsingContext *context, int minPrecedence, Arena *arena, CompilerState *state)
{
    ...
    if (!left)
    {
        parsingError("Expected an expression.", "parseBinaryExpression", table, arena, state, lexer, source);
        return NULL;
    }
    ...
    while (true)
    {
        ...
        if (!right)
        {
            parsingError("Expected an expression on the right side of the operator.", "parseBinaryExpression", table, arena, state, lexer, source);
            return NULL;
        }
        ...
        if (op == OPERATOR_NA)
        {
            parsingError("Invalid operator.", "parseBinaryExpression", table, arena, state, lexer, source);
            return NULL;
        }
...
ASTNode *parseUnaryExpression(Lexer *lexer, CryoSymbolTable *table, ParsingContext *context, Arena *arena, CompilerState *state)
{
    ...
    if (lexer->currentToken.type == TOKEN_MINUS || lexer->currentToken.type == TOKEN_BANG)
    {
        ...
        return createUnaryExpr(opToken, right, arena, state);
    }
    if (lexer->currentToken.type == TOKEN_INCREMENT || lexer->currentToken.type == TOKEN_DECREMENT)
    {
        ...
        if (!right)
        {
            parsingError("Expected an operand", "parseUnaryExpression", table, arena, state, lexer, source);
            return NULL;
        }
    }
...
ASTNode *parsePublicDeclaration(Lexer *lexer, CryoSymbolTable *table, ParsingContext *context, Arena *arena, CompilerState *state)
{
    ...
    default:
        parsingError("Expected a declaration.", "parsePublicDeclaration", table, arena, state, lexer, source);
        return NULL;
    }
}
// </parsePublicDeclaration>
...
ASTNode *parseVarDeclaration(Lexer *lexer, CryoSymbolTable *table, ParsingContext *context, Arena *arena, CompilerState *state)
{
    ...
    if (lexer->currentToken.type != TOKEN_IDENTIFIER)
    {
        ...
        parsingError("[Parser] Expected variable name.", "parseVarDeclaration", table, arena, state, lexer, source);
    }
    ...
    if (lexer->currentToken.type == TOKEN_COLON)
    {
        ...
        {
            parsingError("[Parser] Unknown data type.", "parseVarDeclaration", table, arena, state, lexer, source);
        }
        getNextToken(lexer, arena, state);
    }
    else
    {
        parsingError("[Parser] Expected ':' after variable name.", "parseVarDeclaration", table, arena, state, lexer, source);
    }

    // Parse the variable initializer
    if (lexer->currentToken.type != TOKEN_EQUAL)
    {
        parsingError("[Parser] Expected '=' after type.", "parseVarDeclaration", table, arena, state, lexer, source);
    }
    getNextToken(lexer, arena, state);
    ...
    if (initializer == NULL)
    {
        parsingError("[Parser] Expected expression after '='.", "parseVarDeclaration", table, arena, state, lexer, source);
    }
...
ASTNode *parseFunctionDeclaration(Lexer *lexer, CryoSymbolTable *table, ParsingContext *context, CryoVisibilityType visibility, Arena *arena, CompilerState *state)
{
    ...
    if (lexer->currentToken.type != TOKEN_IDENTIFIER)
    {
        parsingError("Expected an identifier", "parseFunctionDeclaration", table, arena, state, lexer, source);
        return NULL;
    }
    ...
    {
        parsingError("Expected `->` for return type.", "parseFunctionDeclaration", table, arena, state, lexer, source);
    }
    ...
    if (lexer->currentToken.type != TOKEN_LBRACE)
    {
        ...
        parsingError("Expected `{` to start function block.", "parseFunctionDeclaration", table, arena, state, lexer, source);
        return NULL;
    }
    ...
    if (!functionDefNode)
    {
        ...
        parsingError("Failed to create function node.", "parseFunctionDeclaration", table, arena, state, lexer, source);
        return NULL;
    }
    ...
    if (!functionBlock)
    {
        parsingError("Failed to parse function block.", "parseFunctionDeclaration", table, arena, state, lexer, source);
        return NULL;
    }
...
ASTNode *parseExternFunctionDeclaration(Lexer *lexer, CryoSymbolTable *table, ParsingContext *context, Arena *arena, CompilerState *state)
{
    ...
    if (lexer->currentToken.type != TOKEN_IDENTIFIER)
    {
        parsingError("Expected an identifier.", "parseExternFunctionDeclaration", table, arena, state, lexer, source);
        return NULL;
    }
...
ASTNode *parseFunctionCall(Lexer *lexer, CryoSymbolTable *table, ParsingContext *context, char *functionName, Arena *arena, CompilerState *state)
{
    ...
    if (!funcSymbol)
    {
        ...
        parsingError("Function not found or is not a function.", "parseFunctionCall", table, arena, state, lexer, source);
        return NULL;
    }
    ...
    if (lexer->currentToken.type != TOKEN_RPAREN)
    {
        ...
        for (int i = 0; lexer->currentToken.type != TOKEN_RPAREN; ++i)
        {
            ...
            if (!arg)
            {
                ...
                parsingError("Expected argument expression.", "parseFunctionCall", table, arena, state, lexer, source);
                return NULL;
            }
    ...
    if (functionCallNode->data.functionCall->argCount != funcSymbol->argCount)
    {
        ...
        parsingError("Argument count mismatch for function call.", "parseFunctionCall", table, arena, state, lexer, source);
        return NULL;
    }
...
ASTNode *parseParameter(Lexer *lexer, CryoSymbolTable *table, ParsingContext *context, Arena *arena, char *functionName, CompilerState *state)
{
    ...
    if (lexer->currentToken.type != TOKEN_IDENTIFIER)
    {
        parsingError("Expected an identifier.", "parseParameter", table, arena, state, lexer, source);
        return NULL;
    }
...
ASTNode *parseArguments(Lexer *lexer, CryoSymbolTable *table, ParsingContext *context, Arena *arena, CompilerState *state)
{
    ...
    {
        parsingError("Expected an identifier.", "parseArguments", table, arena, state, lexer, source);
        return NULL;
    }
...
ASTNode *parseArgumentsWithExpectedType(Lexer *lexer, CryoSymbolTable *table, ParsingContext *context, CryoDataType expectedType, Arena *arena, CompilerState *state)
{
    ...
    if (lexer->currentToken.type != TOKEN_IDENTIFIER && lexer->currentToken.type != TOKEN_INT_LITERAL && lexer->currentToken.type != TOKEN_STRING_LITERAL && lexer->currentToken.type != TOKEN_BOOLEAN_LITERAL)
    {
        parsingError("Expected an identifier.", "parseArgumentsWithExpectedType", table, arena, state, lexer, source);
        return NULL;
    }
    ...
    else if (lexer->currentToken.type == TOKEN_IDENTIFIER)
    {
        ...
        else
        {
            ...
            parsingError("Symbol not found in the symbol table.", "parseArgumentsWithExpectedType", table, arena, state, lexer, source);
            CONDITION_FAILED;
            return NULL;
        }
    }
...
ASTNode *parseImport(Lexer *lexer, CryoSymbolTable *table, ParsingContext *context, Arena *arena, CompilerState *state)
{
    ...
    if (lexer->currentToken.type != TOKEN_IDENTIFIER)
    {
        parsingError("Expected an identifier.", "parseImport", table, arena, state, lexer, source);
        CONDITION_FAILED;
    }
...
ASTNode *parseExtern(Lexer *lexer, CryoSymbolTable *table, ParsingContext *context, Arena *arena, CompilerState *state)
{
    ...
    default:
        parsingError("Expected an extern declaration.", "parseExtern", table, arena, state, lexer, source);
        return NULL;
    }
...
ASTNode *parseForLoop(Lexer *lexer, CryoSymbolTable *table, ParsingContext *context, Arena *arena, CompilerState *state)
{
    ...
    if (dataType == DATA_TYPE_UNKNOWN)
    {
        parsingError("Unknown data type.", "parseForLoop", table, arena, state, lexer, source);
    }
...
ASTNode *parseArrayIndexing(Lexer *lexer, CryoSymbolTable *table, ParsingContext *context, char *arrayName, Arena *arena, CompilerState *state)
{
    ...
    if (!symbol)
    {
        ...
        parsingError("Array not found.", "parseArrayIndexing", table, arena, state, lexer, source);
        exit(1);
        return NULL;
    }
    else
...
ASTNode *parseAssignment(Lexer *lexer, CryoSymbolTable *table, ParsingContext *context, char *varName, Arena *arena, CompilerState *state)
{
    ...
    if (!symbol)
    {
        ...
        parsingError("Variable not found.", "parseAssignment", table, arena, state, lexer, source);
        return NULL;
    }
    ...
    default:
    {
        ...
        parsingError("Old value is not a variable declaration.", "parseAssignment", table, arena, state, lexer, source);
        return NULL;
    }
    }
    ...
    if (!isMutable)
    {
        ...
        parsingError("Variable is not mutable.", "parseAssignment", table, arena, state, lexer, source);
        return NULL;
    }
...
ASTNode *parseStructDeclaration(Lexer *lexer, CryoSymbolTable *table, ParsingContext *context, Arena *arena, CompilerState *state)
{
    ...
    if (lexer->currentToken.type != TOKEN_IDENTIFIER)
    {
        parsingError("Expected an identifier.", "parseStructDeclaration", table, arena, state, lexer, source);
        return NULL;
    }
    ...
    setThisContext(context, (const char *)structName, NODE_STRUCT_DECLARATION);
...
{
    logMessage("INFO", __LINE__, "Parser", "Parsing `this` context...");
    consume(lexer, TOKEN_KW_THIS, "Expected `this` keyword.", "parseThisContext", table, arena, state);
...
ASTNode *parseDotNotation(Lexer *lexer, CryoSymbolTable *table, ParsingContext *context, Arena *arena, CompilerState *state, ASTNode *left)
{
    ...
    consume(lexer, TOKEN_DOT, "Expected `.` for dot notation.", "parseDotNotation", table, arena, state);

    if (lexer->currentToken.type != TOKEN_IDENTIFIER)
    {
        parsingError("Expected an identifier after `.`.", "parseDotNotation", table, arena, state, lexer, source);
        return NULL;
    }
